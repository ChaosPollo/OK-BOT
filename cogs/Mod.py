    ...
    async def kick_function(self, ctx, user : nextcord.Member = None, *, reason = None):
        ...
        if user == None:
            ...
            if reason == None:
                ...
                text = reason
            e = nextcord.Embed(title=f"{user.name} ha sido expulsado", description=f"Razon: {text}\nFecha de expulsación: {datetime.datetime.utcnow()}\nServidor: {ctx.author.guild}", color=nextcord.Color.from_rgb(30, 255, 165))
            e.set_thumbnail(url=user.display_avatar)
